...
export default function CitizenProfile() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const token = localStorage.getItem("token"); 
        const response = await axios.get(`${apiUrl}/api/citizen/${citizen_id}`, {
          headers: {
            Authorization: `Bearer ${token}` 
          }
        });
        ...
        const registrationResponse = await axios.get(`${apiUrl}/api/registration?cnic=${response.data.citizen_cnic}`, {
          headers: {
            Authorization: `Bearer ${token}` 
          }
        });
